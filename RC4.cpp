...
bool RC4_cipher::encryptDecrypt(const char *inFileName, const char *outFileName, const bool encypting)  {
    ...
    while (!in.eof()) {
        ...
        if (!in) {
            ...
            //или произошла ошибка чтения. Если считалось 0 символов, то это означает пустую строку
            //во входном файле или конец файла. В таком случае переходим к следующей итерации и проверяем eof():
            if (inStr.length() == 0)
                continue;
            else {
